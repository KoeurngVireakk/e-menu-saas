...
export default function AppSheet({ open, title, children, onClose, side = "right" }) {
  if (!open) return null;
  ...
  return (
    <div className="fixed inset-0 z-50 bg-slate-950/45">
      <aside className={`absolute top-0 ${position} flex h-full w-full max-w-md flex-col bg-white shadow-xl`} aria-label={title}>
