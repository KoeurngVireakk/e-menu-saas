...
import { getPreferredLocale, normalizeLocale, setPreferredLocale, supportedLocales, t } from "../../utils/localization";
...
export default function MenuPage() {
  ...
  if (error) {
    return (
      <div className="mx-auto min-h-screen max-w-3xl bg-slate-50" lang={selectedLocale}>
  ...
  if (!menu) {
    return (
      <div className="mx-auto min-h-screen max-w-3xl bg-slate-50" lang={selectedLocale}>
        {!online ? <OfflineBanner locale={selectedLocale} /> : null}
        <div className="p-4">
          <LoadingState message={t(selectedLocale, "loadingMenu")} />
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="mx-auto min-h-screen max-w-3xl bg-slate-50 pb-64" lang={selectedLocale}>
        ...
        <div className="relative px-4 pb-7 pt-10">
          ...
          <div className="flex items-end gap-4">
            ...
            <div className="min-w-0 pb-1">
              <Badge tone="orange">{menu.branch?.name || t(selectedLocale, "mainBranch")}</Badge>
              <h1 className="mt-3 text-3xl font-black leading-tight tracking-tight">{menu.shop.name}</h1>
              <p className="mt-1 text-sm leading-relaxed text-white/75">{menu.table ? `${menu.table.table_name} · ` : ""}{menu.shop.description || t(selectedLocale, "freshMenu")}</p>
            </div>
          </div>
          <Input
            aria-label={t(selectedLocale, "searchMenu")}
            className="mt-5 border-white/20 bg-white text-slate-950 placeholder:text-slate-400"
            placeholder={t(selectedLocale, "searchMenu")}
            value={query}
      ...
      <section className="mt-6 px-4">
        <SectionTitle eyebrow={t(selectedLocale, "menu")} title={activeCategory?.name || t(selectedLocale, "products")} />
      </section>
      <motion.div className="mt-3 grid gap-3 px-4" layout>
        ...
        {!products.length ? <EmptyState title={t(selectedLocale, "noProductsFound")} message={t(selectedLocale, "tryAnotherSearch")} /> : null}
      </motion.div>
      {selected ? <ProductOptionsModal product={selected} locale={selectedLocale} onClose={() => setSelected(null)} onAdd={addConfiguredItem} /> : null}
      <CartDrawer
        cart={cart}
        locale={selectedLocale}
        ...
        onCheckout={() => navigate(`/cart?${checkoutParams.toString()}`)}
      />
    </div>
  );
}

function ProductOptionsModal({ product, locale, onClose, onAdd }) {
  ...
  const submit = async () => {
    ...
    if (missing) {
      toastError(`${missing.name} ${t(locale, "isRequired")}`);
      return;
    }
    ...
    <Modal open onClose={onClose} className="rounded-2xl">
      <div>
        {imageUrl ? (
          ...
          <div className="grid h-36 place-items-center bg-slate-100 text-sm font-semibold text-slate-400">{t(locale, "noImage")}</div>
        )}
        <div className="p-4">
          <div className="flex items-start justify-between gap-3">
            <div>
              <h2 className="text-2xl font-black leading-tight text-slate-950">{product.name}</h2>
              <p className="mt-1 text-sm text-slate-500">{product.description || t(locale, "noDescription")}</p>
            ...
            {(product.options || []).map((option) => (
              <section key={option.id} className="rounded-2xl border border-slate-200 p-3">
                <div className="flex items-center justify-between gap-3">
                  ...
                  {option.is_required ? <Badge tone="orange">{t(locale, "required")}</Badge> : <Badge>{t(locale, "optional")}</Badge>}
                </div>
          ...
          <div className="mt-5 flex items-center justify-between rounded-2xl bg-slate-50 p-3">
            <span className="font-semibold text-slate-800">{t(locale, "quantity")}</span>
          ...
          <div className="sticky bottom-0 mt-4 flex items-center justify-between border-t border-slate-100 bg-white pt-4">
            <div>
              <p className="text-xs uppercase tracking-wide text-slate-500">{t(locale, "total")}</p>
