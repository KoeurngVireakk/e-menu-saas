    ...
    <section className="relative overflow-hidden bg-[linear-gradient(180deg,#F8FAFC_0%,#F2F6FB_52%,#F8FAFC_100%)]">
      ...
      <div className="mx-auto grid max-w-7xl items-center gap-8 px-4 py-10 sm:py-12 lg:grid-cols-[0.92fr_1.08fr] lg:px-8 lg:py-16 xl:py-18">
        <motion.div {...getMotion(reduced)} className="relative z-10 max-w-3xl">
          ...
          <h1 className="khmer-heading mt-5 text-4xl font-black leading-tight tracking-normal text-slate-950 sm:text-5xl lg:text-6xl">
            {t("landing.headline")}
          </h1>
          <p className="khmer-text mt-5 max-w-2xl text-base leading-8 text-slate-600">{t("landing.subheadline")}</p>
          <div className="mt-7 flex flex-col gap-3 sm:flex-row">
            <Link to="/register" className="khmer-button inline-flex min-h-12 items-center justify-center gap-2 rounded-2xl bg-blue-600 px-5 text-sm font-black text-white shadow-sm shadow-blue-600/15 transition hover:-translate-y-0.5 hover:bg-blue-700 active:scale-[0.98]">
              {t("landing.heroPrimary")}
              <ArrowRight className="h-4 w-4" aria-hidden="true" />
            </Link>
            <a href="#customer-preview" className="khmer-button inline-flex min-h-12 items-center justify-center rounded-2xl border border-slate-200 bg-white/75 px-5 text-sm font-black text-slate-800 shadow-sm shadow-slate-900/5 transition hover:-translate-y-0.5 hover:bg-white active:scale-[0.98]">
...
function MobileOrderingStrip() {
  const { t } = useLanguage();
  return (
    <div className="mt-5 flex gap-2 overflow-hidden rounded-3xl border border-slate-200/70 bg-slate-50/70 p-3" aria-label={t("landing.customerFlowEyebrow")}>
...
function PreviewPanel({ icon, title, rows }) {
  return (
    <div className="rounded-3xl border border-slate-200/80 bg-slate-50/70 p-4">
      <div className="flex items-center gap-2">
        <div className="grid h-9 w-9 place-items-center rounded-2xl border border-slate-200/70 bg-white text-blue-700">{icon}</div>
        <h3 className="khmer-heading font-black text-slate-950">{title}</h3>
      </div>
      <div className="mt-3 grid gap-2">
        {rows.map(([label, status]) => (
          <div key={label} className="flex items-center justify-between gap-3 rounded-2xl border border-slate-100 bg-white/90 px-3 py-2 text-sm font-bold text-slate-700">
...
function Section({ id, eyebrow, title, description, children }) {
  ...
  return (
    <motion.section id={id} className="px-4 py-12 sm:py-16 lg:px-8 lg:py-18" {...getMotion(reduced)}>
