...
export default function KitchenQueuePanel({
  title,
  description,
  orders = [],
  loading = false,
  ...
  return (
    <AppCard
      ...
      action={<AppButton as={Link} to="/admin/kitchen" variant="secondary" size="sm">{viewAllLabel}</AppButton>}
      labelled
    >
          ...
          {orders.slice(0, 4).map((order) => (
            ...
            >
              <span className="min-w-0">
                ...
                <span className="mt-1 block truncate text-sm text-slate-500">{order.branch?.name || "Branch"}</span>
              </span>
              <span className="flex shrink-0 flex-wrap gap-2">
                <AppStatusBadge value={order.order_status || "pending"} />
              </span>
            </Link>
          ))}
        </div>
