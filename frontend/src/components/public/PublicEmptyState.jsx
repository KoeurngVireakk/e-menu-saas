...
export default function PublicEmptyState({ title, description, actionLabel, onAction, icon: Icon = ShoppingBag }) {
  return (
    <motion.div
      className="premium-surface rounded-3xl border border-dashed bg-white p-6 text-center"
    ...
    >
      ...
      <h2 className="khmer-heading mt-4 text-lg font-black text-slate-950">{title}</h2>
      {description ? <p className="khmer-text mt-2 text-sm leading-6 text-slate-500">{description}</p> : null}
